...
const SignUp = ({ onFormChange }: { onFormChange?: Function }) => {
    return (
        <div className='top-0 left-0 w-1/2 h-full flex flex-col items-center justify-center p-10'>
            <h1 className='text-4xl font-bold text-black text-center'>Crear Cuenta</h1>
            {/* Social Icons */}
            ...
            <form className='flex flex-col items-center mt-4 min-w-80'>
                ...
                <input
                    ...
                    className='input input-bordered w-full max-w-xs border-gray-400 bg-white text-black mt-4'
                />
                <button className='btn btn-secondary mt-4 text-white font-bold'>Regístrate</button>
            </form>
            <span className='md:hidden text-center text-black mt-4'>
                ¿Ya tienes cuenta?{' '}
                <a className='text-secondary cursor-pointer' onClick={() => onFormChange && onFormChange(false)}>
                    Inicia Sesión
                </a>
            </span>
        </div>
